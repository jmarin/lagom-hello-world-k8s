...
class Lagomhelloworldk8sLoader extends LagomApplicationLoader {
  ...
  override def load(context: LagomApplicationContext): LagomApplication =
    new Lagomhelloworldk8sApplication(context) {
...
abstract class Lagomhelloworldk8sApplication(context: LagomApplicationContext)
  ...
  override lazy val lagomServer: LagomServer = serverFor[Lagomhelloworldk8sService](wire[Lagomhelloworldk8sServiceImpl])

  // Register the JSON serializer registry
  override lazy val jsonSerializerRegistry: JsonSerializerRegistry = Lagomhelloworldk8sSerializerRegistry
